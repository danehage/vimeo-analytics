...
window.addEventListener('beforeunload',function(){player.getCurrentTime().then(function(s){var d={session_id:S,fingerprint_id:F,video_id:V,viewer_id:gvid(),embed_url:location.href,event_type:'session_end',playhead:s,timestamp:new Date().toISOString(),video_duration:D,payload:{seconds:s}};navigator.sendBeacon(EP,new Blob([JSON.stringify(d)],{type:'application/json'}))})});
})();`;
