...
player.on('timeupdate',function(d){P=d.seconds;if(IL&&d.duration>0){if(d.duration===LD){SD++;if(SD>=2){IL=false;D=d.duration}}else{LD=d.duration;SD=0}}if(d.seconds-L>=5){L=d.seconds;send('timeupdate',{seconds:d.seconds,duration:d.duration,percent:d.percent})}});
...
window.addEventListener('beforeunload',function(){var d={session_id:S,fingerprint_id:F,video_id:V,viewer_id:gvid(),embed_url:location.href,event_type:'session_end',playhead:P,timestamp:new Date().toISOString(),video_duration:D,is_live:IL,payload:{seconds:P}};navigator.sendBeacon(EP,new Blob([JSON.stringify(d)],{type:'application/json'}))});
})();`;
